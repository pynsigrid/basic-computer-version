...
for spritePath in list_images(args["sprites"]):  # 读图
    ...
    image = cv2.copyMakeBorder(image, 15, 15, 15, 15,
                               cv2.BORDER_CONSTANT, value=255)  # 画外框

    # invert the image and threshold it
    thresh = cv2.bitwise_not(image)  # 粗分割
    thresh[thresh > 0] = 255
    ...
    cnts = sorted(cnts, key=cv2.contourArea, reverse=True)[0]  # 找最大外框
...
f.write(pickle.dumps(index))  # 存储为cpickle
f.close()
